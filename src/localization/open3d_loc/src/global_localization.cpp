                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    std::string scan_topic = this->declare_parameter("scan_topic", "/cloud_registered_body");
    sub_baselink2odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
        odom_topic, 50, std::bind(&GloabalLocalization::CallbackBaselink2Odom, this, std::placeholders::_1));
    sub_scan_cur_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        scan_topic, 50, std::bind(&GloabalLocalization::CallbackScan, this, std::placeholders::_1));
    ...
    // 适配不同 FAST-LIO 输出: true=里程计frame已经是base_link, false=需要从imu_link转换
    this->declare_parameter<bool>("odom_is_baselink", true);
    // 是否使用 motion_link 帧(旧版luckrobot), 默认不使用
    this->declare_parameter<bool>("use_motion_link", false);
    ...
    bool odom_is_baselink = true;
    bool use_motion_link = false;
    this->get_parameter("odom_is_baselink", odom_is_baselink);
    this->get_parameter("use_motion_link", use_motion_link);
    ...
    else
    {
        ...
        RCLCPP_INFO(this->get_logger(), "odom_is_baselink=true, skip imu_link TF lookup");
    }
    ...
    else
    {
        ...
        RCLCPP_INFO(this->get_logger(), "use_motion_link=false, skip motion_link TF lookup");
    }
...
void GloabalLocalization::CallbackBaselink2Odom(const nav_msgs::msg::Odometry::SharedPtr baselink2odom)
{
    ...
    // 适配不同 FAST-LIO: odom_is_baselink=true 时里程计 frame 已经是 base_link
    // odom_is_baselink=false 时里程计 frame 是 imu_link, 需要 TF 转换
    bool odom_is_baselink = true;
    this->get_parameter("odom_is_baselink", odom_is_baselink);
    if (odom_is_baselink)
    {
    ...
    if (loc_initialized_)
    {
        ...
        geometry_msgs::msg::Pose pose_for_nav2;
        std::string nav2_child_frame;
        ...
        if (use_motion_link)
        {
            ...
            pose_for_nav2 = motionlink2map.pose.pose;
            nav2_child_frame = "motion_link";
        }
        else
        {
            // 直接使用 baselink2map, 不经过 motion_link
            pose_for_nav2 = baselink2map_kalman.pose.pose;
            nav2_child_frame = "base_link";
        }
        ...
        localization_3d_.pose = pose_for_nav2;
...
{
    RCLCPP_INFO(this->get_logger(), "Waiting for odometry and point cloud topics...");
    ...
    while (rclcpp::ok())
    {
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "Waiting for point cloud...");
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
